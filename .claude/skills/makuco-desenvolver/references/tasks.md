...
**Goal**: Break the PBI into GRANULAR, ATOMIC tasks. Clear dependencies. Right tools. Parallel execution plan.
...
### 1. Review the PBI's dev-scoped spec

Read the PBI's `spec.md` (produced earlier in this same flow, inside the PBI's canonical folder) before creating tasks.

### 1.5. Load the Test Coverage Matrix

Read the project's `TESTING.md`, if present, before creating tasks. Its location varies by project — find it via the project's own conventions, don't assume a fixed path. The Test Coverage Matrix and Parallelism Assessment it contains drive two critical decisions:
...
| Code layer with "e2e" requirement         | The `E2E-NN` scenario from `spec.md` implemented + full gate passes |
...
If the project has no TESTING.md (greenfield project, or the convention lives elsewhere), ask the
user what test types and commands the project will use before creating tasks.
...
### 4. Route Each Task to Its Tools

For every task, decide what tools it needs (see the `Tools` field below). When a task touches
backend, frontend, or UI code, the specialist skill to record there — `makuco-backend`,
`makuco-frontend`, or `makuco-ux` — is determined via `gap-routing.md`, not guessed or hardcoded
here.
...
### 6. Validate Before Presenting (MANDATORY)
...
### 7. Confirm MCPs and Skills Per Task

**CRITICAL**: Before execution, confirm — with the user, or by deriving it from the project's own
tool configuration — which tools apply to each task:

> "For each task, which tools should I use?"
>
> **Available MCPs**: [list from project or user]
> **Available Skills**: [list from project or user, including `makuco-backend` / `makuco-frontend` / `makuco-ux` where `gap-routing.md` applies]
...
**Requirement**: [requirement ID from spec.md]
...
**Tests**: [unit/e2e/integration/none — from coverage matrix; for e2e, name the `E2E-NN` of `spec.md`]
...
- MCP: `filesystem`
- Skill: `makuco-frontend` (per `gap-routing.md`)
...
Each sub-agent receives only its task definition and relevant project context (the payload and
return contract are defined in [../steps/step-04-implementar.md](../steps/step-04-implementar.md)
§5). The orchestrating flow waits for all sub-agents in a phase to complete before advancing to
the next phase. A sub-agent is a generic worker running the task's loop — never a `makuco-*`
agent definition.

Sequential tasks (no `[P]`) may be delegated to sub-agents one at a time, or run inline — either
is acceptable as long as the loop is followed unmodified. Delegating keeps implementation
artifacts (file reads, test output, gate check logs) out of the main context.

**The orchestrating flow's role during Execute:**
1. Pick the next task(s) to execute
2. Provide each sub-agent with its task definition + context
3. Monitor sub-agent completion
4. Update task.md with results
...
Before approving tasks, verify EVERY task's `Tests` field is consistent with the project's TESTING.md Test Coverage Matrix (when one exists). This is a hard gate — tasks that fail this check MUST be fixed.
...
- `Tests: none` is only valid when the coverage matrix says "none" for that code layer, or no TESTING.md exists and the user has confirmed no tests are expected.
...
- **Tools per task** — MCPs and Skills prevent wrong approaches; route backend/frontend/UI work to the right specialist skill via `gap-routing.md`
- **Dependencies are gates** — Clear what blocks what
- **Done when = Testable** — If you can't verify it, rewrite it
- **Requirement ID = Traceable** — Every task traces back to a requirement in spec.md
