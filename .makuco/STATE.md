...
- **2026-09-07 — `pbi-001-registro-e-confirmacao-de-conta` NECESSITA CORREÇÕES no code review (rodada 1)** — 1 critical (CORS não configurado em `services/backend/src/main.ts` — o fluxo de cadastro nunca funciona num navegador real, só nos testes automatizados, que não passam pela política de CORS) + 1 major (endpoint `POST /auth/register` sem rate limiting). Ver `pbis/pbi-001-registro-e-confirmacao-de-conta/review.md` para os 18 achados completos. Volta para `makuco-desenvolver` antes de seguir para `pbi-002`.
